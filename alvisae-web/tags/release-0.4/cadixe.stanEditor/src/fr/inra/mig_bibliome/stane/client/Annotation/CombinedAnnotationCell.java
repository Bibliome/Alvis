...
public abstract class CombinedAnnotationCell extends AbstractCell<Annotation> {
    ...
    public static interface AnnotationCellTemplates extends SafeHtmlTemplates {
        ...
        @Template("<span aae_refannid='{0}' style='background-color:silver;'>{1}</span> ")
        public SafeHtml annRefSpan(String annotationId, String displayId);

        @Template("<span aae_refannid='{0}' title='{0}' style='background-color:silver;'>{1}</span> ")
        ...
        @Template("<span aae_refannid='{0}' title='missing referenced Annotation \"{0}\"' style='color:red;'>{1}</span> ")
    ...
    public static SafeHtml getKindImage(AnnotationKind kind) {
        ...
        switch (kind) {
            case TEXT:
                image = StanEditorResources.INSTANCE.TextAnnotationIcon();
                break;
            case GROUP:
                image = StanEditorResources.INSTANCE.GroupAnnotationIcon();
                break;
            case RELATION:
                image = StanEditorResources.INSTANCE.RelationAnnotationIcon();
                break;
        }
        return SafeHtmlUtils.fromSafeConstant(AbstractImagePrototype.create(image).getHTML());
    }
