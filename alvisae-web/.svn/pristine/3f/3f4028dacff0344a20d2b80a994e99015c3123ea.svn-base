...
import fr.inra.mig_bibliome.stane.client.data.DetailedAsyncCallback;
...
public class UserManager extends Composite implements UserManagingView {
    ...
    public UserManager() {
        ...
        selectionModel = new SingleSelectionModel<EditableExtendedUserInfoImpl>(KEY_PROVIDER);
        ...
        userInfoProvider = new ListDataProvider<EditableExtendedUserInfoImpl>(KEY_PROVIDER);
    ...
    private void initAuthTable() {
        ...
        injector.getCoreDataProvider().getCampaignList(new AsyncCallback<CampaignListImpl>() {
            ...
            public void onSuccess(CampaignListImpl result) {
                for (int row = 0; row < result.length(); row++) {
                    campaigns.put(result.get(0).getId(), result.get(0).getDisplayName());
                }
            }
        });
