...
import fr.inra.mig_bibliome.stane.client.Events.InformationReleasedEvent;
import fr.inra.mig_bibliome.stane.client.SignIn.ChangePasswordDialog;
import fr.inra.mig_bibliome.stane.client.StaneCoreResources;
import fr.inra.mig_bibliome.stane.client.Start.DefaultPlace;
import fr.inra.mig_bibliome.stane.client.User.PasswordUpdater;
import fr.inra.mig_bibliome.stane.client.User.UserManagingParams;
...
public class CampaignDocList extends Composite implements DocSelectingView {
    ...
    private void refreshCampaignList(final Command executedOnSuccess) {
        ...
        userActionMenu.setTitle("authenticated as: " + injector.getCoreDataProvider().getRequestManager().getCurrentUserName());
    ...
    public CampaignDocList() {
        ...
        userActionMenuItem.setHTML(SafeHtmlUtils.fromSafeConstant(AbstractImagePrototype.create(StaneCoreResources.INSTANCE.SignedInIcon()).getHTML()));
        ...
        shbuilder = new SafeHtmlBuilder();
        shbuilder.append(SafeHtmlUtils.fromSafeConstant(AbstractImagePrototype.create(StaneCoreResources.INSTANCE.ManageUsersIcon()).getHTML())).appendEscaped("  Manager users");
        item = subMenuBar.addItem(shbuilder.toSafeHtml(), new Command() {
    ...
    private void manageUsers() {
        presenter.goTo(new UserManagingPlace(new UserManagingParams(injector.getCoreDataProvider().getRequestManager().getCurrentUserId())));
    }
