...
/**
 * Table displaying the annotations associated to the currently selected document 
 * @author fpapazian
 */
public class AnnotationTable extends Composite implements WorkingDocumentChangedEventHandler, EditHappenedEventHandler, AnnotationSelectionChangedEventHandler {
    ...
    interface TableStyle extends DragAndDropCellTable.Style {
    }

    interface TableResources extends DragAndDropCellTable.Resources {

        @Source({DragAndDropCellTable.Style.DEFAULT_CSS, "AnnotationTable.css"})
        @Override
    ...
    DragAndDropCellTable<Annotation> annotationsGrid;
    @UiField
...
//
    ...
    private HashMap<Integer, String> annSetInfos = new HashMap<Integer, String>();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    public AnnotationTable() {
        annotationsGrid = new DragAndDropCellTable<Annotation>(15, resources);
    ...
    private void initTable() {
        ...
        annotationsGrid.addColumn(idColumn, "Id");
    ...
    private void resetColumnSize(boolean showTermColumn) {
        annotationsGrid.setWidth("98%", true);
        ...
        annotationsGrid.setColumnWidth(veiledColumn, 26.0, Unit.PX);
    }
    ...
    private void reinitAnnSetFilterMenu() {
        ...
        for (Entry<Integer, String> e : annSetInfos.entrySet()) {
            ...
            if (authorizedAnnSetIds.contains(annSetId)) {
                ...
                shbuilder.appendEscaped(e.getValue());
                annSetMenuBar.addItem(shbuilder.toSafeHtml(), command);
            } else {
                MenuItem item = new MenuItem(e.getValue(), (Command) null);
    ...
    private void displayAnnotationList(AnnotatedTextHandler workingDocument, DocumentView docView, boolean initialLoad) {
        ...
        if (lastWorkingDocument == null) {
            ...
            for (AnnotationSetInfo asi : lastWorkingDocument.getAnnotatedText().getAnnotationSetInfoList()) {
                annSetInfos.put(asi.getId(), asi.getDescription());
            }
    ...
    private void filterAnnotationListAndRefresh(Set<Integer> displayAnnSetIds) {
        ...
        for (Annotation a : annotationSrc) {
            if (isTermAnnotation(a)) {
                ...
                annotationList.add(new TermAnnotationBox(a, tyDITermRefPropName, tyDIClassRefPropName));
            } else {
                ...
                if (selectedIds.contains(annotationId)) {
                    ...
                    if (mainSelectedId.contains(annotationId)) {
                        ...
                        try {
                            ...
                            if (index != null) {
                                scrollPanel.setVerticalScrollPosition(annotationsGrid.getRowElement(index).getOffsetTop());
                            } else {
