...
import fr.inra.mig_bibliome.stane.client.Edit.AnnotationGroupCreationEdit;
import fr.inra.mig_bibliome.stane.client.Edit.AnnotationGroupEditionEdit;
...
public class EditGroupDialog extends GenericEditDialog {
    ...
    public static void startGroupCreation(final AnnotatedTextHandler annotatedDoc, List<String> annotationIds) {
        ...
        dlg.setApplyCommand(new Command() {
            ...
            public void execute() {
                ...
                String groupType = dlg.getAnnotationType();
                List<Annotation> components = dlg.getComponents();
    ...
    private final BasicFaultListener faultLstnr;
    ...
    protected void validateAnnotation() {
        ...
        if (checkCompleteness()) {
            ...
            Annotation annotation = ((AnnotatedTextImpl) annotatedText).createLooseGroupAnnotation("new group", typeName, getComponents());
    ...
    private List<Annotation> getComponents() {
        List<Annotation> components = new ArrayList<Annotation>();
        for (int row = 1; row < argumentsGrid.getRowCount(); row++) {
            CheckBox ckbox = (CheckBox) argumentsGrid.getWidget(row, 1);
            if (ckbox.getValue()) {
                String id = model.get(row - 1);
                Annotation annotation = getAnnotatedTextHandler().getAnnotation(id);
