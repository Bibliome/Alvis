...
import fr.inra.mig_bibliome.stane.client.Annotation.EditRelationDialog.ExclusiveSelection.ExclusiveChangeHandler;
import fr.inra.mig_bibliome.stane.client.Edit.AnnotationRelationCreationEdit;
import fr.inra.mig_bibliome.stane.client.Edit.AnnotationRelationEditionEdit;
...
public class EditRelationDialog extends GenericEditDialog {
    ...
    public static void startRelationCreation(final AnnotatedTextHandler annotatedDoc, List<String> annotationIds) {
        ...
        dlg.setApplyCommand(new Command() {
            ...
            public void execute() {
                ...
                String relType = dlg.getAnnotationType();
                Map<String, Annotation> argumentRoleMap = dlg.getArgumentRoleMap();
    ...
    private BasicFaultListener faultLstnr;
    ...
    public EditRelationDialog(AnnotatedTextHandler annotatedText) {
        ...
        faultLstnr = new BasicFaultListener();
    }
    ...
    private void setRelationToCreate(List<String> annotationIds) {
        ...
        ArrayPermuter permutations = new ArrayPermuter<String>(permAnnIds);
        ...
        while (!foundMatching && permutations.next()) {
            ...
            int typeIndex = 0;
            for (AnnotationTypeDefinition type : getAnnotationTypes()) {
    ...
    protected void prepareArgumentGrid(List<String> annotationIds) {
        ...
        AnnotationTypeDefinition selectedType = getAnnotationTypeDef();
        List<String> roles;
    ...
    protected void validateAnnotation() {
        ...
        if (checkCompleteness()) {
            ...
            faultLstnr.reset();
            String typeName = getAnnotationType();
            AnnotationTypeDefinition annTypeDef = annotatedText.getAnnotationSchema().getAnnotationTypeDefinition(typeName);
            Annotation annotation = ((AnnotatedTextImpl) annotatedText).createLooseRelationAnnotation("new relation", typeName, getArgumentRoleMap());
    ...
    private Map<String, Annotation> getArgumentRoleMap() {
        Map<String, Annotation> argumentRole = new HashMap<String, Annotation>();
        for (int row = 1; row < argumentsGrid.getRowCount(); row++) {
            String role = argumentsGrid.getText(row, 0);
            ListBox lb = (ListBox) argumentsGrid.getWidget(row, 1);
            Annotation annotation = getAnnotatedTextHandler().getAnnotation(lb.getValue(lb.getSelectedIndex()));
            argumentRole.put(role, annotation);
        }
        return argumentRole;
    }
}
